...
namespace BepInEx_DemulShooter_Plugin
{
    ...
    {
        public const String pluginGuid = "argonlefou.demulshooter.mib";
        public const String pluginName = "MIB_BepInEx_DemulShooter_Plugin";
        public const String pluginVersion = "2.0.0.0";
        public const String pluginConfigFile = "MIB_BepInEx_DemulShooter_Plugin.ini";
        ...
        {            
            ...
            if (MIBCore.Instance != null)
            {
                MIBCore.Instance.InputPosition[0] = PluginControllers[0].GetAimingPosition();
                MIBCore.Instance.InputPosition[1] = PluginControllers[1].GetAimingPosition();
            }

            //COIN KEYS
            if (Coin_Key.GetButtonDown())
            {
                MIBCore.Instance.UniversalCredits++;
                if (MIBCore.Instance.CreditsController != null)
                {
                    MIBCore.Instance.CreditsController.UpdateCredit(0, MIBCore.Instance.UniversalCredits);
                }
            }
            ...
            for (int i = 0; i < MAX_PLAYERS; i++)
            {
                if (PluginControllers[i].GetButtonDown(PluginController.MyInputButtons.Start))
                    HandleGameStart(i + 1);
            }
            ...
            for (int i = 0; i < MAX_PLAYERS; i++)
            {
                try
                {
                    OutputData.IsPlaying[i] = Global.IsPlayerAI(i) ? (byte)0 : (byte)1;
                } catch{ }
                //OutputData.Ammo[i] = Global.GetGun();
            }
        ...
        private void HandleGameStart(int playerIndex)
        {
            if (MIBCore.Instance == null)
            {
                return;
            }
            CreditsUI creditsUI = UnityEngine.Object.FindObjectOfType<CreditsUI>();
            if (creditsUI == null)
            {
                return;
            }
            AttractScreen attractScreen = UnityEngine.Object.FindObjectOfType<AttractScreen>();
            if (attractScreen != null)
            {
                if (MIBCore.Instance.FreePlay || MIBCore.Instance.UniversalCredits >= MIBCore.Instance.GameCost)
                {
                    if (!MIBCore.Instance.FreePlay)
                    {
                        MIBCore.Instance.UniversalCredits -= MIBCore.Instance.GameCost;
                    }
                    creditsUI.UpdateCredit(0, MIBCore.Instance.UniversalCredits);
                    attractScreen.CreditUsed(new CreditInfo(creditsUI.gameObject, playerIndex));
                }
                return;
            }
            EndScreenAP endScreenAP = UnityEngine.Object.FindObjectOfType<EndScreenAP>();
            if (endScreenAP != null)
            {
                if (MIBCore.Instance.FreePlay || MIBCore.Instance.UniversalCredits >= MIBCore.Instance.ContinueCost)
                {
                    if (!MIBCore.Instance.FreePlay)
                    {
                        MIBCore.Instance.UniversalCredits -= MIBCore.Instance.ContinueCost;
                    }
                    creditsUI.UpdateCredit(0, MIBCore.Instance.UniversalCredits);
                    endScreenAP.CreditUsed(new CreditInfo(creditsUI.gameObject, playerIndex));
                }
                return;
            }
            if ((MIBCore.Instance.Players & playerIndex) == 0 && MIBCore.Instance.Players != 3)
            {
                if (MIBCore.Instance.FreePlay || MIBCore.Instance.UniversalCredits >= MIBCore.Instance.GameCost)
                {
                    if (!MIBCore.Instance.FreePlay)
                    ...
                    creditsUI.UpdateCredit(0, MIBCore.Instance.UniversalCredits);
                    MIBCore.Instance.PlayerPressedStart(playerIndex);
                }
                return;
            }
        }
