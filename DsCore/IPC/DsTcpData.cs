...
namespace DsCore.IPC
{
    ...
    public class DsTcpData
    {
        ...
        /// </summary>
        public DsTcpData(int PlayerNumber)
        {
            try
            {
                ...
                foreach (FieldInfo fi in _DataFields)
                {
                    ...
                    if (fi.FieldType.IsArray && fi.GetValue(this) == null)
                    {
                        ...
                        Logger.WriteLog(this.GetType().ToString() + " : Creating " + fi.FieldType.GetElementType().ToString() + " [" + PlayerNumber + "] " + fi.Name);
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.WriteLog("OutputData.Ctor(): " + ex.Message.ToString());
            }
        }
        ...
        public void Update(byte[] ReceivedData)
        {
                ...
                {
                    s += "0x" + ReceivedData[i].ToString("X2") + " ";
                }
                Logger.WriteLog("DsTcpData.Update(): Received byte array " + s);
            #endif
                    ...
                    foreach (FieldInfo fi in _DataFields)
                    {
                        if (fi.FieldType.IsArray)
                        {
                            ...
                            for (int i = 0; i < a.Length; i++)
                            {
                                ...
                                #if DEBUG_DSTCPDATA
                                    Logger.WriteLog("DsTcpData.Update(): " + fi.Name + "[" + i + "] = " + a.GetValue(i).ToString());
                                #endif
                            }
                        }
                        else
                        {
                            ...
                            #if DEBUG_DSTCPDATA
                                Logger.WriteLog("DsTcpData.Update(): " + fi.Name + " = " + fi.GetValue(this).ToString());
                            #endif
                        }
                    }
                }
        ...
        private void PrivateDebug(string s)
        {
#if DEBUG_DSTCPDATA
